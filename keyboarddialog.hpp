...
static const char* BARK_DIALOG_WINDOW_ID = "L:D_N:dialog_PC:T_ID:com.teknoist.barkplayer";
static const int BARK_BOOKMARK_TITLE_MAX = 32;

void show_dialog_keyboard(std::string& text) {
    GtkWidget* dialog = gtk_dialog_new();
    gtk_window_set_title(GTK_WINDOW(dialog), BARK_DIALOG_WINDOW_ID);
    ...
    gtk_entry_set_max_length(GTK_ENTRY(entry), BARK_BOOKMARK_TITLE_MAX);
        ...
        for (int i = 0; letters[i] != '\0'; i++) {
            ...
            g_signal_connect(key_button, "clicked",
                           G_CALLBACK(+[](GtkWidget*, gpointer user_data) {
                               ...
                               if ((int)text.length() < BARK_BOOKMARK_TITLE_MAX) {
                                   text += kd->letter;
